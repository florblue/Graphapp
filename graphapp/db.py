...
from graphapp.contextmanager import Neo4jDBSessionManager
...
""" def create_node(handle_id, name, description, label):
    q = 'CREATE (p:Person { name: {name}, description: {description} })'
    with manager.session as s:
...
def wildcard_search(search_string):
    ...
    q = """
        ...
        OPTIONAL MATCH (p:Person) WHERE p.name =~ {search_string} WITH collect(p) as persons
        RETURN persons
        """
    with manager.session as s:
